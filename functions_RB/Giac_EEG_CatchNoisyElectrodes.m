...
switch mode
    case 'recursive'
        ...
        while strcmp(Go_recursive,'yes') == 1
            ...
            [ trial_flat,indices_flat] = find(round(mean(mean_mat,1), 5) == 0);      % RB  find the channels that have zero activity
            [ trial_flat_std, indices_flat_std] = find(round(mean(std_mat,1), 5) < 0.2);  % RB find the channels that have zero activity
