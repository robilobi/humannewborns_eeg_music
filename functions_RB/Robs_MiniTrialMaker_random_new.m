function [all_erp_all_midi] = Robs_MiniTrialMaker_random_new(data,midi_names,code_corresponding_to_midi,baseline_dur,new_sampling,n_offsets, allnotes)
%
...
%     cfg.trials       = instances_of_same_midi';
    ...
    %make it random
    n=note_onsets*1000;
    randonsets= randperm(floor(max(n)), length(note_onsets));
    note_onsets= sort((randonsets)/1000)';
    ...
    for ii = 1:length(instances_of_same_midi) % loop into same exposure of 1 song
        ...
        EEG_epochs_of_one_midi.trialinfo(:,6) = notesinfo(:,3);  % condition
