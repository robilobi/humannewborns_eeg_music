function [signal, ref_section, eigen_ref_section, sample_mask] = RFT_clean_asr(signal,cutoff,windowlen,stepsize,maxdims,ref_maxbadchannels,ref_tolerances,ref_wndlen,usegpu,useriemannian,maxmem)
...
if isnumeric(ref_maxbadchannels) && isnumeric(ref_tolerances) && isnumeric(ref_wndlen)
    ...
    try
        [ref_section, sample_mask] = clean_windows(signal,ref_maxbadchannels,ref_tolerances,ref_wndlen); %% RB: output sample mask to compute % signal retained
    catch e
...
if exist('hlp_diskcache','file')
    ...
    if useriemannian
        ...
        [state , eigen_ref_section] = RFT_asr_calibrate(ref_section.data,ref_section.srate,cutoff, [], [], [], [], [], [], [], maxmem);
    end
end
%clear ref_section;
